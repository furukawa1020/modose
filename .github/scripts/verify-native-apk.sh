...
methods=(nativeCreate nativeApply nativeClose nativeBeginVerification nativeCompleteVerification)
...
for apk in "$@"; do
  ...
  for abi in "${abis[@]}"; do
    ...
    printf 'verified %s %s: ELF ABI, five JNI exports, 16KB LOAD alignment\n' "$apk" "$abi"
  done
done
